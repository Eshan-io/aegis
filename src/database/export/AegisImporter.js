...
export default class AegisImporter extends Importer {
    ...
    checkAllTypes() {
        ...
        if (this.data.levels !== undefined) {
            if (!(this.data.levels instanceof Array)) {
                throw new TypeError('Levels must be an array');
            }
            for (const level of this.data.levels) {
                level.guildid = this.guildID;
                Level.checkTypes(level);
            }
        }
    }
    ...
    async _importGuildConfig() {
        ...
        this.data.guildConfig.leveling ??= {};
        this.data.guildConfig.leveling.rewards ??= [];
        this.data.guildConfig.leveling.rewards = await asyncFilter(
            this.data.guildConfig.leveling.rewards,
            async reward => !!await guildWrapper.fetchRole(reward.roleid)
        );
    ...
    async _importLevels() {
        ...
        await Level.bulkSave(levels.map(level => new Level({
    ...
    generateEmbed() {
        return new EmbedBuilder()
            ...
            .addFields(
                ...
                // eslint-disable-next-line jsdoc/reject-any-type
                /** @type {any} */{ name: 'Levels', value: (this.data.levels ?? []).length.toString(), inline: true },
            );
    }
}
