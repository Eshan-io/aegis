...
export default class ModerationListCommand extends SubCommand {
    ...
    async generateMessage(member, moderations, page = 1) {
        ...
        for (const data of [
            { id: `case:user:${member.user.id}:first`, emoji: 'firstPage', label: icons.first, disabled: page === 1 },
            { id: `case:user:${member.user.id}:${page - 1}`, emoji: 'previousPage', label: icons.left, disabled: page === 1 },
            { id: `case:user:${member.user.id}:${page}`, emoji: 'refresh', label: icons.refresh },
            { id: `case:user:${member.user.id}:${page + 1}`, emoji: 'nextPage', label: icons.right, disabled: page === lastPage },
            { id: `case:user:${member.user.id}:last`, emoji: 'lastPage', label: icons.last, disabled: page === lastPage },
        ]) {
